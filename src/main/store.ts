...
export function getReasoningMap(projectId: string): ReasoningMap | null {
  const maps = getStore().get('reasoningMaps');
  return maps[projectId] ?? null;
}

export function setReasoningMap(projectId: string, map: ReasoningMap): void {
  const s = getStore();
  const maps = s.get('reasoningMaps');
  maps[projectId] = map;
  s.set('reasoningMaps', maps);
}

export function getFileAttribution(projectId: string): Record<string, { agent: AgentType | 'user'; date: string }> {
  const all = getStore().get('fileAttribution');
  return all[projectId] ?? {};
}

export function updateFileAttribution(projectId: string, files: string[], agent: AgentType | 'user'): void {
  const s = getStore();
  const all = s.get('fileAttribution');
  const current = all[projectId] ?? {};
  const date = new Date().toISOString();
  for (const file of files) {
    current[file] = { agent, date };
  }
  all[projectId] = current;
  s.set('fileAttribution', all);
}

// --- Installed Skills ---

export function getInstalledSkills(projectId: string): InstalledSkillRecord[] {
  const all = getStore().get('installedSkills');
  return all[projectId] ?? [];
}

export function addInstalledSkill(projectId: string, record: InstalledSkillRecord): void {
  const s = getStore();
  const all = s.get('installedSkills');
  const existing = all[projectId] ?? [];
  const filtered = existing.filter((r) => r.skillId !== record.skillId);
  all[projectId] = [...filtered, record];
  s.set('installedSkills', all);
}

export function removeInstalledSkill(projectId: string, skillId: string): void {
  const s = getStore();
  const all = s.get('installedSkills');
  const existing = all[projectId] ?? [];
